...
const StaffApplications = () => {
  ...
  const loadSports = useCallback(async () => {
    ...
    try {
      ...
      setSports(normalizedRows);
      if (normalizedRows.length > 0) {
        setSelectedSportId((previous) => {
          if (previous && normalizedRows.some((sport) => Number(sport.id) === Number(previous))) {
            return previous;
          }
          return String(normalizedRows[0].id);
        });
      } else {
        setSelectedSportId("");
      }
      ...
      setSports([]);
      setSelectedSportId("");
      setError(summarizeApiError(apiError, "Unable to load sports."));
    } finally {
  ...
  const loadApplications = useCallback(async (sportId) => {
    ...
    try {
      ...
      setApplications([]);
      setAssignedStaff([]);
      setError(summarizeApiError(apiError, "Failed to load staff applications for selected sport."));
    } finally {
  ...
  return (
    <div className="os-page-shell">
      <PageHeaderCard
        ...
        subtitle="Review staff and official applications across sports."
        breadcrumbs="Coordinator / Staff Applications"
        icon={ClipboardList}
        action={
          <select
      ...
      <div className="mb-4 rounded-xl border border-amber-200 bg-amber-50 px-4 py-3 text-sm text-amber-800 dark:border-amber-500/40 dark:bg-amber-500/10 dark:text-amber-200">
        <p className="font-semibold">Primary workflow moved</p>
        <p className="mt-1">
          Staff applications are no longer part of the primary workflow. Assign staff and officials directly in User Management.
        </p>
        <Link to="/coordinator/management/users?section=staff" className="mt-2 inline-flex text-xs font-semibold text-blue-700 hover:underline dark:text-blue-300">
      ...
      {loadingSports ? (
        ...
        <EmptyState
          ...
          message="Staff applications cannot be reviewed because no sports were found."
        />
      ) : (
        <>
          ...
          <DashboardCard>
            ...
            {loadingRows ? (
              ...
              <EmptyState
                ...
                message="No applications found for this sport and filter."
              />
            ) : (
              <DataTable
                columns={[
